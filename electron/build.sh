...
echo "Building Lurus API Electron App..."
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    ...
    CGO_ENABLED=1 go build -ldflags="-s -w" -o lurus-api ./cmd/server
    cd electron
    npm install
    npm run build:mac
    ...
    CGO_ENABLED=1 go build -ldflags="-s -w" -o lurus-api.exe ./cmd/server
    cd electron
    npm install
    npm run build:win
else
